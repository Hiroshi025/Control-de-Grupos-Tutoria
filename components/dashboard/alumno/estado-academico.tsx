...
export function EstadoAcademico({ alumno }: EstadoAcademicoProps) {
  const pieData = [
    { name: "Aprobadas", value: alumno.materias_aprobadas, color: "hsl(var(--chart-1))" },
    { name: "En Recurso", value: alumno.materias_en_recurso, color: "hsl(var(--destructive))" },
    { name: "Sin Cursar", value: alumno.materias_sin_cursar, color: "hsl(var(--chart-3))" },
    { name: "Especial", value: alumno.materias_en_especial, color: "hsl(var(--chart-4))" },
  ]
          ...
          <div className="space-y-4">
            <ChartContainer
              config={{
                ...
                recurso: { label: "En Recurso", color: "hsl(var(--destructive))" },
                sinCursar: { label: "Sin Cursar", color: "hsl(var(--chart-3))" },
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-4 bg-chart-1/10 rounded-lg">
                <p className="text-2xl font-bold text-chart-1">{alumno.materias_aprobadas}</p>
                <p className="text-sm text-muted-foreground">Aprobadas</p>
              </div>
              <div className="text-center p-4 bg-destructive/10 rounded-lg">
                <p className="text-2xl font-bold text-destructive">{alumno.materias_en_recurso}</p>
                <p className="text-sm text-muted-foreground">En Recurso</p>
              </div>
              <div className="text-center p-4 bg-chart-3/10 rounded-lg">
                <p className="text-2xl font-bold text-chart-3">{alumno.materias_sin_cursar}</p>
                <p className="text-sm text-muted-foreground">Sin Cursar</p>
              </div>
              <div className="text-center p-4 bg-chart-4/10 rounded-lg">
                <p className="text-2xl font-bold text-chart-4">{alumno.materias_en_especial}</p>
